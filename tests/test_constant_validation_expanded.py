"""Expanded test validation of constants in the plant integration."""
...
def test_constants_defined():
    ...
    assert const.DEVICE_TYPE_TENT == "tent"  # Added tent device type
...
def test_config_flow_constants():
    ...
    # Test tent config flow constants
    assert const.SERVICE_CREATE_TENT == "create_tent"
    assert const.SERVICE_CHANGE_TENT == "change_tent"
    assert const.SERVICE_UPDATE_TENT_SENSORS == "update_tent_sensors"


def test_tent_constants():
    """Test that tent-related constants are defined."""
    _setup_ha_modules()
...
def test_default_aggregations():
    """Test that default aggregations are properly defined."""
    _setup_ha_modules()
    
    # Load the const module
    const = _load_module("custom_components.plant.const", "custom_components/plant/const.py")
    
    # Test that DEFAULT_AGGREGATIONS contains required keys
    assert "temperature" in const.DEFAULT_AGGREGATIONS
    assert "moisture" in const.DEFAULT_AGGREGATIONS
    assert "conductivity" in const.DEFAULT_AGGREGATIONS
    assert "illuminance" in const.DEFAULT_AGGREGATIONS
    assert "humidity" in const.DEFAULT_AGGREGATIONS
    assert "CO2" in const.DEFAULT_AGGREGATIONS
    assert "ppfd" in const.DEFAULT_AGGREGATIONS
    assert "dli" in const.DEFAULT_AGGREGATIONS
    ...
    assert "health" in const.DEFAULT_AGGREGATIONS
    assert "ph" in const.DEFAULT_AGGREGATIONS


def test_device_types():
    """Test that device types are properly defined."""
    _setup_ha_modules()
    
    # Load the const module
    const = _load_module("custom_components.plant.const", "custom_components/plant/const.py")
    
    # Test that DEVICE_TYPES contains required values
    assert const.DEVICE_TYPE_PLANT in const.DEVICE_TYPES
    assert const.DEVICE_TYPE_CYCLE in const.DEVICE_TYPES
    assert const.DEVICE_TYPE_TENT in const.DEVICE_TYPES
    # Note: DEVICE_TYPE_CONFIG is not in DEVICE_TYPES as it's not for user selection


if __name__ == "__main__":
    test_constants_defined()
    test_service_constants()
    test_consumption_constants()
    test_config_flow_constants()
    test_tent_constants()
    test_default_aggregations()
    test_device_types()
    print("All expanded constant validation tests passed!")