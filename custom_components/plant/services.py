...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def replace_sensor(call: ServiceCall) -> None:
        ...
        Raises:
            HomeAssistantError: If the meter entity is not found, invalid sensor is provided,
                               or any other error occurs during the replacement process
        """
        try:
            ...
            if not meter_entity:
                _LOGGER.warning("No meter entity specified for replacement")
                raise HomeAssistantError("No meter entity specified for replacement")
            ...
            if not found:
                ...
                raise HomeAssistantError(f"Entity {meter_entity} not found in {DOMAIN}")

            if new_sensor and new_sensor != "" and not new_sensor.startswith("sensor."):
                _LOGGER.warning("%s is not a sensor", new_sensor)
                raise HomeAssistantError(f"{new_sensor} is not a valid sensor entity")

            try:
                meter = hass.states.get(meter_entity)
            except Exception as e:
                _LOGGER.error("Error getting meter entity %s: %s", meter_entity, str(e))
                raise HomeAssistantError(f"Error accessing meter entity {meter_entity}: {str(e)}")
                
            if meter is None:
                _LOGGER.error("Meter entity %s not found", meter_entity)
                raise HomeAssistantError(f"Meter entity {meter_entity} not found")

            if new_sensor and new_sensor != "":
                try:
                    test = hass.states.get(new_sensor)
                except Exception as e:
                    _LOGGER.error("Error getting new sensor entity %s: %s", new_sensor, str(e))
                    raise HomeAssistantError(f"Error accessing new sensor entity {new_sensor}: {str(e)}")
                    
                if test is None:
                    _LOGGER.error("New sensor entity %s not found", new_sensor)
                    raise HomeAssistantError(f"New sensor entity {new_sensor} not found")
            else:
                new_sensor = None
                    ...
                    for meter in meters:
                        if meter.entity_id == meter_entity:
                            try:
                                ...
                                _LOGGER.error("Error replacing sensor for %s: %s", meter_entity, str(e))
                                raise HomeAssistantError(f"Error replacing sensor for {meter_entity}: {str(e)}")
                                
            if replacement_count == 0:
                _LOGGER.warning("No sensors were replaced for %s", meter_entity)
            else:
                _LOGGER.info("Successfully replaced %d sensors for %s", replacement_count, meter_entity)
                
        except HomeAssistantError:
            # Re-raise HomeAssistantError as-is
            raise
        except Exception as e:
            _LOGGER.error("Unexpected error in replace_sensor service: %s", str(e))
            raise HomeAssistantError(f"Unexpected error in replace_sensor service: {str(e)}")

    async def remove_plant(call: ServiceCall) -> None:
        """Remove a plant entity and all its associated entities.
        
        This service removes a plant and all its associated entities (sensors, thresholds, etc.)
        from Home Assistant. It also handles cleanup of cycle associations if the plant was
        part of a cycle.
        
        Args:
            call: ServiceCall object containing:
                - plant_entity (str): The entity ID of the plant to remove
        
        Raises:
            HomeAssistantError: If the plant entity is not found or any error occurs
        ...
        try:
            ...
            if not plant_entity:
                _LOGGER.warning("No plant entity specified for removal")
                raise HomeAssistantError("No plant entity specified for removal")
            ...
            if not found:
                ...
                raise HomeAssistantError(f"Plant entity {plant_entity} not found in {DOMAIN}")

            try:
                # Check if the Plant is assigned to a Cycle and update its phase
                            ...
                            for entry_id in hass.data[DOMAIN]:
                                if ATTR_PLANT in hass.data[DOMAIN][entry_id]:
                                    ...
                                    if (cycle.device_type == DEVICE_TYPE_CYCLE and 
                                        ...
                                        try:
                                            # Remove the Plant from the Cycle
                                            cycle.remove_member_plant(plant_entity)
                                            # Update Flowering Duration
                                            if cycle.flowering_duration:
                                                await cycle.flowering_duration._update_cycle_duration()
                                        except Exception as e:
                                            _LOGGER.warning("Error updating cycle after plant removal: %s", str(e))
                                        break
                            break

                # Remove the Config Entry
                await hass.config_entries.async_remove(target_entry_id)
                _LOGGER.info("Successfully removed plant entity: %s", plant_entity)
                return True
                
            except Exception as e:
                _LOGGER.error("Error removing plant entity %s: %s", plant_entity, str(e))
                raise HomeAssistantError(f"Error removing plant entity {plant_entity}: {str(e)}")
                
        except HomeAssistantError:
            # Re-raise HomeAssistantError as-is
            raise
        except Exception as e:
            _LOGGER.error("Unexpected error in remove_plant service: %s", str(e))
            raise HomeAssistantError(f"Unexpected error in remove_plant service: {str(e)}")

    async def create_plant(call: ServiceCall) -> ServiceResponse:
        """Create a new plant.
        
        This service creates a new plant entity with associated sensors and thresholds.
        It can optionally fetch plant data from OpenPlantbook if available.
        
        Args:
            call: ServiceCall object containing:
                - name (str): The name of the plant
                - strain (str): The strain of the plant
                - breeder (str, optional): The breeder of the plant
                - growth_phase (str, optional): The current growth phase
                - plant_emoji (str, optional): Emoji to represent the plant
                - temperature_sensor (str, optional): Entity ID of temperature sensor
                - moisture_sensor (str, optional): Entity ID of moisture sensor
                - conductivity_sensor (str, optional): Entity ID of conductivity sensor
                - illuminance_sensor (str, optional): Entity ID of illuminance sensor
                - humidity_sensor (str, optional): Entity ID of humidity sensor
                - co2_sensor (str, optional): Entity ID of CO2 sensor
                - power_consumption_sensor (str, optional): Entity ID of power consumption sensor
                - ph_sensor (str, optional): Entity ID of pH sensor
        
        Returns:
            ServiceResponse: Contains the entity_id of the created plant
        
        Raises:
            HomeAssistantError: If required parameters are missing or any error occurs
                               during the creation process
        """
        try:
            # Validate required parameters
            if not call.data.get(ATTR_NAME):
                raise HomeAssistantError("Plant name is required")
                
            if not call.data.get(ATTR_STRAIN):
                raise HomeAssistantError("Plant strain is required")

            try:
                # Create a complete plant_info object
                plant_info = {
                ...
                sensor_mapping = {
                    ...
                    FLOW_SENSOR_CO2: call.data.get(FLOW_SENSOR_CO2),
                    FLOW_SENSOR_POWER_CONSUMPTION: call.data.get(FLOW_SENSOR_POWER_CONSUMPTION),
                    FLOW_SENSOR_PH: call.data.get(FLOW_SENSOR_PH),
                }
                
                for sensor_key, sensor_value in sensor_mapping.items():
                    if sensor_value:
                        # Validate sensor entity ID format
                        if not sensor_value.startswith("sensor."):
                            _LOGGER.warning("Invalid sensor entity ID format: %s", sensor_value)
                            raise HomeAssistantError(f"Invalid sensor entity ID: {sensor_value}")
                        plant_info[sensor_key] = sensor_value

                # Get data from OpenPlantbook
                ...
                try:
                    ...
                    _LOGGER.warning("Error fetching plant data from OpenPlantbook: %s", str(e))
                    # Continue with manual creation if Plantbook fails
                    plant_config = None
                ...
                if plant_config and plant_config.get(FLOW_PLANT_INFO, {}).get(DATA_SOURCE) == DATA_SOURCE_PLANTBOOK:
                    try:
                        ...
                        for sensor_key in [FLOW_SENSOR_TEMPERATURE, FLOW_SENSOR_MOISTURE, FLOW_SENSOR_CONDUCTIVITY, 
                                          ...
                                          FLOW_SENSOR_CO2, FLOW_SENSOR_PH]:
                            if sensor_key in plant_info and plant_info[sensor_key]:
                                opb_info[sensor_key] = plant_info[sensor_key]
                            elif sensor_key in opb_info and not opb_info[sensor_key]:
                                # Remove empty sensor assignments from Plantbook
                                opb_info.pop(sensor_key, None)
                        
                        plant_info = opb_info
                    except Exception as e:
                        _LOGGER.warning("Error processing Plantbook data: %s", str(e))
                        # Fall back to manual plant info if processing fails
                        pass

                # Create config entry
                try:
                    entry = hass.config_entries.flow.async_init(
                        DOMAIN,
                        context={"source": "create_plant"},
                        data={FLOW_PLANT_INFO: plant_info}
                    )
                    result = await entry
                    _LOGGER.info("Successfully created plant: %s", plant_info[ATTR_NAME])
                    return {"entity_id": result["result"].entry_id}
                except Exception as e:
                    _LOGGER.error("Error creating config entry for plant: %s", str(e))
                ...
                _LOGGER.error("Error in create_plant service: %s", str(e))
                raise HomeAssistantError(f"Error creating plant: {str(e)}")
            ...
            _LOGGER.error("Unexpected error in create_plant service: %s", str(e))
            raise HomeAssistantError(f"Unexpected error creating plant: {str(e)}")
    ...
    async def create_cycle(call: ServiceCall) -> ServiceResponse:
        ...
        try:
            ...
            cycle_name = call.data.get(ATTR_NAME)
            if not cycle_name:
                cycle_name = "New Cycle"  # Default name if none provided
                
            cycle_info = {
                ATTR_NAME: cycle_name,
    ...
    async def remove_cycle(call: ServiceCall) -> bool:
        """Remove a cycle and all its associated entities."""
        cycle_entity_id = call.data.get("cycle_entity")
        
        if not cycle_entity_id:
            _LOGGER.warning("No cycle entity specified for removal")
            return False
        ...
        for entry_id in hass.data[DOMAIN]:
            if ATTR_PLANT in hass.data[DOMAIN][entry_id]:
                cycle = hass.data[DOMAIN][entry_id][ATTR_PLANT]
                if cycle.entity_id == cycle_entity_id and cycle.device_type == DEVICE_TYPE_CYCLE:
                    found = True
                    target_entry_id = entry_id
                    target_cycle = cycle
                    break
        ...
        if not found:
            _LOGGER.warning(
                "Refuse to remove non-%s cycle entity: %s", DOMAIN, cycle_entity_id
            )
            return False

        try:
            # Remove all plants from this cycle
            try:
                device_registry = dr.async_get(hass)
                cycle_device = device_registry.async_get_device(
                    identifiers={(DOMAIN, target_cycle.unique_id)}
                )
                
                if cycle_device:
                    # Find all plants assigned to this cycle
                    for device in device_registry.devices.values():
                        if device.via_device_id == cycle_device.id:
                            # This plant is assigned to the cycle, remove it
                            for entry_id in hass.data[DOMAIN]:
                                if ATTR_PLANT in hass.data[DOMAIN][entry_id]:
                                    plant = hass.data[DOMAIN][entry_id][ATTR_PLANT]
                                    if plant.device_id == device.id:
                                        # Remove plant from cycle
                                        plant._cycle = None
                                        # Update device registry
                                        device_registry.async_update_device(
                                            device.id,
                ...
                _LOGGER.warning("Could not update device registry: %s", str(e))
                # Continue with removal even if device registry update fails

            # Remove the Config Entry
            await hass.config_entries.async_remove(target_entry_id)
            _LOGGER.info("Successfully removed cycle entity: %s", cycle_entity_id)
            return True
            
        except Exception as e:
            _LOGGER.error("Error removing cycle entity %s: %s", cycle_entity_id, str(e))
            return False

    async def watering_completed(target_plant) -> None:
        """Callback function when watering is completed."""
        try:
            # Update plant state to indicate watering is complete
            if hasattr(target_plant, 'last_watering_completed'):
                target_plant.last_watering_completed = dt_util.utcnow()
                target_plant.async_write_ha_state()
        except Exception as e:
            _LOGGER.error("Error in watering_completed callback: %s", str(e))

    async def add_watering(call: ServiceCall) -> None:
        """Add a manual watering entry to a plant's journal and update total water consumption."""
        ...
        # Add journal entry
        if hasattr(target_plant, 'journal'):
            await target_plant.journal.async_set_value(f"Watering: {amount_liters}L - {note}")

        # Update total water consumption
        if hasattr(target_plant, 'total_water_consumption'):
            # Check if the method exists
            if hasattr(target_plant.total_water_consumption, 'add_manual_watering'):
                # Call the add_manual_watering method (it might be async or sync)
                try:
                    result = target_plant.total_water_consumption.add_manual_watering(amount_liters, note)
                    if asyncio.iscoroutine(result):
                        await result
                except Exception:
                    # Fallback to direct increment if method fails
                    try:
                        target_plant.total_water_consumption += amount_liters
                    except Exception:
                        pass
            else:
                # Fallback to direct increment if method not available
                try:
                    target_plant.total_water_consumption += amount_liters
                except Exception:
                    pass
    ...
    async def add_ph(call: ServiceCall) -> None:
        ...
        # Update pH sensor
        # First check for sensor_ph attribute (primary)
        if hasattr(target_plant, "sensor_ph") and target_plant.sensor_ph:
            ...
            if hasattr(sensor, "set_manual_value"):
                ...
                try:
                    ...
                    pass
        # Then check for ph attribute (alternative)
        elif hasattr(target_plant, "ph") and target_plant.ph:
    ...
    async def reassign_to_tent(call: ServiceCall) -> None:
        ...
        try:
            ...
            _LOGGER.info(f"Successfully assigned plant {plant_entity_id} to tent {tent_entity_id}")
    ...
    async def clone_plant(call: ServiceCall) -> ServiceResponse:
        ...
        if not source_entity_id:
            raise HomeAssistantError("Source plant entity ID is required")
        ...
        # Get source plant config
        source_entry = hass.config_entries.async_get_entry(source_entry_id)
        source_config = dict(source_entry.data)
        source_plant_info = dict(source_config.get(FLOW_PLANT_INFO, {}))
        
        # Create new plant info based on source
        new_plant_info = source_plant_info.copy()
        new_plant_info[ATTR_NAME] = new_name if new_name else f"{source_plant_info.get(ATTR_NAME, 'Plant')} Clone"
        new_plant_info[ATTR_IS_NEW_PLANT] = True
        
        # Clear any unique identifiers that shouldn't be copied
        new_plant_info.pop("entity_id", None)
        new_plant_info.pop("unique_id", None)
        
        # Create new config entry
        new_config = {FLOW_PLANT_INFO: new_plant_info}
        ...
        try:
            ...
            if result["type"] == FlowResultType.CREATE_ENTRY:
                return {
                    "success": True,
                    "new_plant_entity_id": result["result"].entry_id,
                    "message": f"Plant cloned successfully as {new_plant_info[ATTR_NAME]}"
                }
            else:
                raise HomeAssistantError(f"Failed to clone plant: {result.get('reason', 'unknown error')}")
                
        except Exception as e:
            _LOGGER.error(f"Error cloning plant: {e}")
            raise HomeAssistantError(f"Error cloning plant: {e}")

    async def export_plants(call: ServiceCall) -> ServiceResponse:
        """Export plant configurations to a ZIP archive."""
        plant_entity_ids = call.data.get("plant_entities", [])
        file_path = call.data.get("file_path", "/config/plants_export.zip")
        include_images = call.data.get("include_images", False)
        include_sensor_data = call.data.get("include_sensor_data", False)
        ...
        try:
            ...
            # Process each plant
            for plant_entity_id in plant_entity_ids:
                # Find the plant config entry
                found_entry = None
                for entry in hass.config_entries.async_entries(DOMAIN):
                    if (entry.data.get(FLOW_PLANT_INFO, {}).get("entity_id") == plant_entity_id or 
                        entry.entry_id in plant_entity_id):
                        found_entry = entry
                        break
                
                if not found_entry:
                    _LOGGER.warning(f"Plant entity {plant_entity_id} not found")
                    continue
                
                plant_info = found_entry.data.get(FLOW_PLANT_INFO, {})
        ...
        try:
            ...
            for plant_data in import_data["plants"]:
                try:
                    ...
                    if result["type"] == FlowResultType.CREATE_ENTRY:
                        imported_count += 1
                        _LOGGER.info(f"Successfully imported plant: {plant_name}")
                    else:
                        error_msg = result.get("reason", "unknown error")
                        errors.append(f"Failed to import {plant_name}: {error_msg}")
                        _LOGGER.error(f"Failed to import plant {plant_name}: {error_msg}")
                        
                except Exception as e:
                    error_msg = str(e)
                    errors.append(f"Error importing {original_name}: {error_msg}")
                    _LOGGER.error(f"Error importing plant {original_name}: {e}")
            
            # Prepare response
            response_data = {
                "imported_plants": imported_count,
                "skipped_plants": skipped_count,
