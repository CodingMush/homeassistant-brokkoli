...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def remove_cycle(call: ServiceCall) -> None:
        ...
        if not cycle_entity_id:
            ...
            raise HomeAssistantError("No cycle entity specified for removal")
        ...
        if not found:
            ...
            raise HomeAssistantError(f"Cycle entity {cycle_entity_id} not found in {DOMAIN}")
        ...
        try:
            ...
            raise HomeAssistantError(f"Error removing cycle entity {cycle_entity_id}: {str(e)}")

    async def clone_plant(
        hass: HomeAssistant, new_name: str, plant_entity_id: str
    ) -> dict:
        """Clone a plant from a plant entity."""
        _LOGGER.debug("Clone plant with entity_id: %s", plant_entity_id)
        plant_info = await get_plant_info(hass, plant_entity_id)

        if not plant_info:
            _LOGGER.error(
                "Failed to get info for plant entity: %s", plant_entity_id
            )
            return {"error": "Failed to get info for plant entity."}

        # Copy plant data to a new entity
        plant_data = plant_info["plant_data"]
        new_plant_entity_id = await hass.services.async_call(
            DOMAIN,
            SERVICE_ADD_PLANT,
            {
                "name": new_name,
                "interval": plant_data["interval"],
                "moisture_max": plant_data["moisture_max"],
                "moisture_min": plant_data["moisture_min"],
                "brightness_min": plant_data["brightness_min"],
                "brightness_max": plant_data["brightness_max"],
            },
            blocking=True,
        )

        if not new_plant_entity_id:
            _LOGGER.error("Failed to clone plant: %s", new_name)
            return {"error": "Failed to clone plant."}

        _LOGGER.info("Plant cloned: %s", new_name)
    ...
    async def handle_clone_plant(call: ServiceCall) -> ServiceResponse:
        ...
        if not new_name:
            ...
            if source_state:
                source_name = source_state.attributes.get("friendly_name", "Plant")
                new_name = f"{source_name} Clone"
            else:
                new_name = "Cloned Plant"
        
        result = await clone_plant(hass, new_name, source_entity_id)
        return result
    ...
    async def update_plant_attributes(call: ServiceCall) -> None:
        ...
        if not target_entry or not target_plant:
    ...
    async def add_watering(call: ServiceCall) -> None:
        ...
        # Add journal entry
        journal = target_plant.journal
        await journal.add_entry(
            JournalEntry(
                event=JournalEntryEvent.WATERING,
                timestamp=dt_util.utcnow(),
                data={JournalEntryEvent.WATERING.value: amount_liters, "note": note},
            )
        )

        # Update total water consumption
        target_plant.total_water_consumed += amount_liters

        await hass.helpers.event.async_call_later(
            timedelta(minutes=target_plant.watering_interval),
            functools.partial(watering_completed, target_plant=target_plant),
        )
